...
class uMCTS:
  ...
  def select_child(self, node: Node):
    ...
    for _, child in node.children.items():
      ...
      score = child.q_value + c * math.sqrt(math.log(node.visit_count + 1) / (child.visit_count + 1))
      if score > best_score:
        best_score = score
        best_child = child
    return best_child
  ...
  def compute_policy(self, root: Node):
    ...
    policy = {action: child.visit_count / total_visits for action, child, in root.children.items()}
    return policy