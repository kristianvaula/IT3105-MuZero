...
def test_network_builder():
    ...
    assert isinstance(model, torch.nn.Sequential), (
        "Network is not an instance of nn.Sequential"
    )
...
def test_neural_network_init(neural_network):
    assert neural_network.network is not None, (
        "NeuralNetwork instance failed to initialize"
    )
...
def test_load_model(neural_network):
    ...
    for param1, param2 in zip(
        neural_network.network.parameters(), new_nn_model.network.parameters()
    ):
...
def test_forward_pass(neural_network):
    ...
    assert output.shape == (1, 64), (
        f"Forward pass output shape is incorrect: {output.shape}"
    )
...
def test_model_rebuild_on_load():
    ...
    assert new_nn_model.network is not None, (
        "Network was not rebuilt before loading weights"
    )
